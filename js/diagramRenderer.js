...
export class DiagramRenderer {
  ...
  getDiagramPrintLabelLayout(hole, { ignoreOffset = false } = {}) {
    ...
    const edgeGapPx = Number.isFinite(configuredDistance) ? Math.max(-15, Math.min(20, configuredDistance)) : 8;
  ...
  getDiagramPrintCornerLabelLayout(hole, { ignoreOffset = false } = {}) {
    ...
    const angleDeg = Number.isFinite(configuredAngle) ? configuredAngle : 315;
    const configuredDistance = Number(this.stateRef?.ui?.labelDistancePx);
    const edgeGapPx = (Number.isFinite(configuredDistance) ? Math.max(-15, Math.min(20, configuredDistance)) : 8) + 18;
    const radians = (angleDeg * Math.PI) / 180;
    const ux = Math.sin(radians);
    const uy = -Math.cos(radians);
    const orbitPadding = Math.hypot(metrics.width / 2, metrics.height / 2);
  ...
  drawTimingVisualization(preview) {
    ...
    const activeIndex = this.stateRef?.ui?.activeTimingPreviewIndex;
